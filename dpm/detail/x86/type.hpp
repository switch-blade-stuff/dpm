...
#pragma region "simd operators"
	...
	DPM_DECLARE_EXT_NAMESPACE
	{
		...
		[[nodiscard]] DPM_FORCEINLINE detail::x86_simd<T, N, A> lsl(const detail::x86_simd<T, M, A> &x) noexcept requires (detail::x86_overload_any<T, M, A> && sizeof(T) > 1 && std::numeric_limits<T>::digits < N)
		{
			detail::x86_simd<T, N, A> result = {};
		...
		template<std::size_t N, std::integral T, std::size_t M, std::size_t A>
		[[nodiscard]] DPM_FORCEINLINE detail::x86_simd<T, N, A> lsr(const detail::x86_simd<T, M, A> &x) noexcept requires (detail::x86_overload_any<T, M, A> && sizeof(T) > 1 && std::numeric_limits<T>::digits < N)
		{
			detail::x86_simd<T, N, A> result = {};
		...
		[[nodiscard]] DPM_FORCEINLINE detail::x86_simd<T, N, A> asl(const detail::x86_simd<T, M, A> &x) noexcept requires (detail::x86_overload_any<T, M, A> && sizeof(T) > 1 && std::numeric_limits<T>::digits < N)
		{
			detail::x86_simd<T, N, A> result = {};
			detail::vectorize([](auto &res, auto x) { res = detail::bit_shiftl<T, N>(x); }, result, x);
			return result;
		...
		template<std::size_t N, std::signed_integral T, std::size_t M, std::size_t A>
		[[nodiscard]] DPM_FORCEINLINE detail::x86_simd<T, N, A> asr(const detail::x86_simd<T, M, A> &x) noexcept requires (detail::x86_overload_any<T, M, A> && sizeof(T) > 1 && std::numeric_limits<T>::digits < N)
		{
			detail::x86_simd<T, N, A> result = {};
