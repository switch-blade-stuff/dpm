...
namespace dpm
{
	...
	[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<T, N, A> pow(const detail::x86_simd<T, N, A> &x, const detail::x86_simd<T, N, A> &p) noexcept
	{
		if (std::is_constant_evaluated())
		{
			const auto packed_x = static_cast<simd<T, simd_abi::ext::packed_buffer<N>>>(x);
			const auto packed_p = static_cast<simd<T, simd_abi::ext::packed_buffer<N>>>(p);
			return static_cast<detail::x86_simd<T, N, A>>(pow(packed_x, packed_p));
		}
		else
		{
	...
	template<std::floating_point T, std::size_t N, std::size_t A> requires detail::x86_overload_any<T, N, A>
	[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<T, N, A> pow(const detail::x86_simd<T, N, A> &x, T p) noexcept
	{
		if (std::is_constant_evaluated())
		{
			const auto packed = static_cast<simd<T, simd_abi::ext::packed_buffer<N>>>(x);
			return static_cast<detail::x86_simd<T, N, A>>(pow(packed, p));
		}
		else
		{
			detail::x86_simd<T, N, A> result = {};
	...
	template<std::floating_point T, std::size_t N, std::size_t A> requires detail::x86_overload_any<T, N, A>
	[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<T, N, A> cbrt(const detail::x86_simd<T, N, A> &x) noexcept
	{
		if (std::is_constant_evaluated())
		{
			const auto packed = static_cast<simd<T, simd_abi::ext::packed_buffer<N>>>(x);
			return static_cast<detail::x86_simd<T, N, A>>(cbrt(packed));
		}
		else
		{
			detail::x86_simd<T, N, A> result = {};
	...
	template<std::floating_point T, std::size_t N, std::size_t A> requires detail::x86_overload_any<T, N, A>
	[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<T, N, A> sqrt(const detail::x86_simd<T, N, A> &x) noexcept
	{
		if (std::is_constant_evaluated())
		{
			const auto packed = static_cast<simd<T, simd_abi::ext::packed_buffer<N>>>(x);
			return static_cast<detail::x86_simd<T, N, A>>(sqrt(packed));
		}
		else
		{
			detail::x86_simd <T, N, A> result = {};
	...
	template<std::floating_point T, std::size_t N, std::size_t A> requires detail::x86_overload_any<T, N, A>
	[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<T, N, A> hypot(const detail::x86_simd<T, N, A> &a, const detail::x86_simd<T, N, A> &b) noexcept
	{
		if (std::is_constant_evaluated())
		{
			...
			return static_cast<detail::x86_simd<T, N, A>>(hypot(packed_a, packed_b));
		}
		else
		{
			detail::x86_simd <T, N, A> result = {};
	...
	DPM_DECLARE_EXT_NAMESPACE
	{
		...
		template<std::floating_point T, std::size_t N, std::size_t A> requires detail::x86_overload_any<T, N, A>
		[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<T, N, A> rcp(const detail::x86_simd<T, N, A> &x) noexcept
		{
			if (std::is_constant_evaluated())
			{
				const auto packed = static_cast<simd<T, simd_abi::ext::packed_buffer<N>>>(x);
				return static_cast<detail::x86_simd<T, N, A>>(rcp(packed));
			}
			else
			{
				detail::x86_simd <T, N, A> result = {};
		...
		template<std::size_t N, std::size_t A> requires detail::x86_overload_any<float, N, A>
		[[nodiscard]] constexpr DPM_FORCEINLINE detail::x86_simd<float, N, A> rsqrt(const detail::x86_simd<float, N, A> &x) noexcept
		{
			if (std::is_constant_evaluated())
			{
				const auto packed = static_cast<simd<float, simd_abi::ext::packed_buffer<N>>>(x);
				return static_cast<detail::x86_simd<float, N, A>>(rsqrt(packed));
			}
			else
			{
