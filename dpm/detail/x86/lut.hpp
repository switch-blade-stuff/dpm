...
namespace dpm::detail
{
	...
	[[nodiscard]] inline const V &assert_lut_idx(std::index_sequence<Is...>, const V &v_idx, [[maybe_unused]] DPM_SOURCE_LOC_TYPE loc) noexcept
	{
	...
	[[nodiscard]] inline const V &assert_lut_idx(const V &v_idx, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
	...
	[[nodiscard]] DPM_FORCEINLINE V lut_load(std::span<const float, N> data, __m128i v_idx, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
	...
	[[nodiscard]] DPM_FORCEINLINE V lut_load(std::span<const float, N> data, __m256i v_idx, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
	...
	[[nodiscard]] DPM_FORCEINLINE V lut_load(std::span<const double, N> data, __m128i v_idx, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
	...
	[[nodiscard]] DPM_FORCEINLINE V lut_load(std::span<const double, N> data, __m256i v_idx, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
	...
	[[nodiscard]] DPM_FORCEINLINE V lut_load(const float *data, __m256i idx, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
		return _mm256_castps128_ps256(lut_load<__m128, Idx, Scale>(data, idx, loc));
	}
	template<std::same_as<__m256> V, integral_of_size<8> Idx, int Scale = 1>
	[[nodiscard]] DPM_FORCEINLINE V lut_load(const float *data, __m256i idxl, __m256i idxh, DPM_SOURCE_LOC_TYPE loc) noexcept
	{
