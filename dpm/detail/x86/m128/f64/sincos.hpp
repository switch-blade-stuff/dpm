...
namespace dpm
{
	...
	[[nodiscard]] inline simd<double, detail::avec<N, A>> sin(const simd<double, detail::avec<N, A>> &x) noexcept requires detail::x86_overload_128<double, N, A>
	{
	...
	template<std::size_t N, std::size_t A>
	[[nodiscard]] inline simd<double, detail::avec<N, A>> cos(const simd<double, detail::avec<N, A>> &x) noexcept requires detail::x86_overload_128<double, N, A>
	{
		simd<double, detail::avec<N, A>> result = {};
		for (std::size_t i = 0; i < ext::native_data_size_v<simd<double, detail::avec<N, A>>>; ++i)
	...
	{
		template<std::size_t N, std::size_t A>
		inline void sincos(const simd<double, detail::avec<N, A>> &x, simd<double, detail::avec<N, A>> &out_sin, simd<double, detail::avec<N, A>> &out_cos) noexcept requires detail::x86_overload_128<double, N, A>
		{
			for (std::size_t i = 0; i < ext::native_data_size_v<simd<double, detail::avec<N, A>>>; ++i)
			{
				const auto [sin, cos] = detail::sincos(ext::to_native_data(x)[i]);
