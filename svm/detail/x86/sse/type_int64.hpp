...
namespace svm
{
	namespace detail
	{
		...
		struct x86_mask_impl<I, __m128i, N>
		{
			...
			static SVM_SAFE_ARRAY void copy_from(const bool *src, __m128i *dst, F) noexcept
			{
			...
			template<std::size_t M, typename F>
			static SVM_SAFE_ARRAY void copy_to(bool *dst, const __m128i *src, F) noexcept
			{
				for (std::size_t i = 0; i < N; i += 2)
				{
			...
			template<std::size_t M, typename F>
			static SVM_SAFE_ARRAY void copy_from(const bool *src, __m128i *dst, const __m128i *mask, F) noexcept
			{
				for (std::size_t i = 0; i < N; i += 2)
				{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M, typename F>
			static SVM_SAFE_ARRAY void copy_to(bool *dst, const __m128i *src, const __m128i *mask, F) noexcept
			{
				for (std::size_t i = 0; i < N; i += 2)
				{
			...
			static SVM_SAFE_ARRAY void invert(__m128i *dst, const __m128i *src) noexcept
			{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			static SVM_SAFE_ARRAY void bit_and(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			static SVM_SAFE_ARRAY void bit_or(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			static SVM_SAFE_ARRAY void bit_xor(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			static SVM_SAFE_ARRAY void cmp_eq(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
#ifdef SVM_HAS_SSE2
				...
				for (std::size_t i = 0; i < M; ++i)
				{
					...
					out[i] = std::bit_cast<__m128i>(_mm_cmpeq_ps(va, vb));
				}
#endif
			}
			template<std::size_t M>
			static SVM_SAFE_ARRAY void cmp_ne(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
#ifdef SVM_HAS_SSE2
				...
				for (std::size_t i = 0; i < M; ++i)
				{
					const auto va = _mm_and_ps(std::bit_cast<__m128>(a[i]), nan_mask);
					const auto vb = _mm_and_ps(std::bit_cast<__m128>(b[i]), nan_mask);
					out[i] = std::bit_cast<__m128i>(_mm_cmpneq_ps(va, vb));
				}
#endif
			}

			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY bool all_of(const __m128i *mask) noexcept
			{
			...
			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY bool any_of(const __m128i *mask) noexcept
			{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY bool none_of(const __m128i *mask) noexcept
			{
#ifdef SVM_HAS_SSE2
				auto result = _mm_setzero_si128();
				for (std::size_t i = 0; i < M; ++i)
			...
			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY bool some_of(const __m128i *mask) noexcept
			{
#ifdef SVM_HAS_SSE4_1
			...
			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY std::size_t popcount(const __m128i *mask) noexcept
			{
				std::size_t result = 0;
				for (std::size_t i = 0; i < M; ++i)
				{
			...
			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY std::size_t find_first_set(const __m128i *mask) noexcept
			{
				for (std::size_t i = 0; i < M; ++i)
				{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			[[nodiscard]] static SVM_SAFE_ARRAY std::size_t find_last_set(const __m128i *mask) noexcept
			{
...
#ifdef SVM_HAS_SSE4_1
			template<std::size_t M>
			static SVM_SAFE_ARRAY void blend(__m128i *out, const __m128i *a, const __m128i *b, const __m128i *m) noexcept
			{
		...
		template<integral_of_size<8> I, std::size_t N>
		struct x86_simd_impl<I, __m128i, N>
		{
			template<typename U>
			...
			static SVM_SAFE_ARRAY void copy_from(const U *src, __m128i *dst, F) noexcept
			{
			...
			template<std::size_t M, typename U, typename F>
			static SVM_SAFE_ARRAY void copy_to(U *dst, const __m128i *src, F) noexcept
			{
				if constexpr (std::same_as<U, I> && aligned_tag<F, alignof(__m128i)>)
					for (std::size_t i = 0; i < N; i += 2)
					{
			...
			template<std::size_t M, typename U, typename F>
			static SVM_SAFE_ARRAY void copy_from(const U *src, __m128i *dst, const __m128i *mask, F) noexcept
			{
#ifdef SVM_HAS_AVX
				if constexpr (std::same_as<U, I> && aligned_tag<F, alignof(__m128i)>)
					for (std::size_t i = 0; i < N; i += 2)
...
#endif
					for (std::size_t i = 0; i < N; ++i) if (data_at<std::uint64_t>(mask, i)) data_at<I>(dst, i) = static_cast<I>(src[i]);
			}
			template<std::size_t M, typename U, typename F>
			static SVM_SAFE_ARRAY void copy_to(U *dst, const __m128i *src, const __m128i *mask, F) noexcept
			{
#ifdef SVM_HAS_AVX
				if constexpr (std::same_as<U, I> && aligned_tag<F, alignof(__m128i)>)
					for (std::size_t i = 0; i < N; i += 2)
...
#endif
						for (std::size_t i = 0; i < N; ++i) if (data_at<std::uint64_t>(mask, i)) dst[i] = static_cast<U>(data_at<I>(src, i));
				}
			}

			template<std::size_t M>
			static SVM_SAFE_ARRAY void inc(__m128i *data) noexcept
			{
#ifdef SVM_HAS_SSE2
			...
			template<std::size_t M>
			static SVM_SAFE_ARRAY void dec(__m128i *data) noexcept
			{
#ifdef SVM_HAS_SSE2
				const auto one = _mm_set1_epi64x(1);
			...
			template<std::size_t M>
			static SVM_SAFE_ARRAY void negate(__m128i *dst, const __m128i *src) noexcept
			{
#ifdef SVM_HAS_SSE2
...
#ifdef SVM_HAS_SSE2
			template<std::size_t M>
			static SVM_SAFE_ARRAY void add(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
				for (std::size_t i = 0; i < M; ++i) out[i] = _mm_add_epi64(a[i], b[i]);
			}
			template<std::size_t M>
			static SVM_SAFE_ARRAY void sub(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
				for (std::size_t i = 0; i < M; ++i) out[i] = _mm_sub_epi64(a[i], b[i]);
			}

			template<std::size_t M>
			static SVM_SAFE_ARRAY void lshift(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
				for (std::size_t i = 0; i < M; ++i) out[i] = _mm_sll_epi64(a[i], b[i]);
			}
			template<std::size_t M>
			static SVM_SAFE_ARRAY void rshift(__m128i *out, const __m128i *a, const __m128i *b) noexcept
			{
	...
	{
		template<detail::integral_of_size<8> I, std::size_t N, std::size_t Align> requires simd_abi::detail::x86_overload_sse<I, N, Align>
	...
	template<detail::integral_of_size<8> I, std::size_t N, std::size_t Align> requires detail::x86_overload_sse<I, N, Align>
	...
	public:
		...
		constexpr SVM_SAFE_ARRAY simd_mask(__m128i native) noexcept requires (data_size == 1) { m_data[0] = native; }
		...
		constexpr SVM_SAFE_ARRAY simd_mask(const __m128i (&native)[data_size]) noexcept { std::copy_n(native, data_size, m_data); }
	...
	namespace detail
	{
		template<detail::integral_of_size<8> I, std::size_t N, std::size_t A> requires detail::x86_overload_sse<I, N, A>
		struct native_access<simd_mask<I, avec<N, A>>>
		{
			using mask_t = simd_mask<I, avec<N, A>>;

			[[nodiscard]] static std::span<__m128i, mask_t::data_size> to_native_data(mask_t &value) noexcept { return {value.m_data}; }
			[[nodiscard]] static std::span<const __m128i, mask_t::data_size> to_native_data(const mask_t &value) noexcept { return {value.m_data}; }
		};
	}
...
#pragma endregion
	...
	public:
		...
		constexpr SVM_SAFE_ARRAY simd(__m128i native) noexcept requires (data_size == 1) { m_data[0] = native; }
		...
		constexpr SVM_SAFE_ARRAY simd(const __m128i (&native)[data_size]) noexcept { std::copy_n(native, data_size, m_data); }
		...
		{
#ifdef SVM_HAS_SSE2
			const auto vec = _mm_set1_epi64x(std::bit_cast<I>(static_cast<I>(value)));
		...
		SVM_SAFE_ARRAY simd(G &&gen) noexcept
		{
			...
			detail::generate_n<data_size>(m_data, [&gen]<std::size_t J>(std::integral_constant<std::size_t, J>)
			{
				...
				{
					default: 1 = std::invoke(gen, std::integral_constant<std::size_t, value_idx + 1>());
					case 1: 0 = std::invoke(gen, std::integral_constant<std::size_t, value_idx>());
				}
		...
		/** Copies the underlying elements from \a mem. */
		template<typename U, typename Flags>
		SVM_SAFE_ARRAY void copy_from(const U *mem, Flags) noexcept requires is_simd_flag_type_v<Flags> { impl_t::template copy_from<data_size>(mem, m_data, Flags{}); }
		/** Copies the underlying elements to \a mem. */
		template<typename U, typename Flags>
		SVM_SAFE_ARRAY void copy_to(U *mem, Flags) const noexcept requires is_simd_flag_type_v<Flags> { impl_t::template copy_to<data_size>(mem, m_data, Flags{}); }
		...
		[[nodiscard]] mask_type SVM_SAFE_ARRAY operator!() const noexcept
		{
			mask_type result;
		...
		[[nodiscard]] simd SVM_SAFE_ARRAY operator-() const noexcept requires std::is_signed_v<I>
		{
			simd result;
	...
	namespace detail
	{
		...
		struct native_access<simd<I, avec<N, A>>>
		{
			...
			[[nodiscard]] static std::span<__m128i, simd_t::data_size> to_native_data(simd_t &value) noexcept { return {value.m_data}; }
			[[nodiscard]] static std::span<const __m128i, simd_t::data_size> to_native_data(const simd_t &value) noexcept { return {value.m_data}; }
		};
	}
